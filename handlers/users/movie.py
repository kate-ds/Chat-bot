...
async def process_movie_details(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await message.bot.send_message(message.chat.id, msg)
        # await message.bot.send_message(message.chat.id, "Хочешь оставить отзыв о фильме?")


# # Leave a review for a movie
# @dp.message_handler(lambda message: ("да" in message.text.lower()), state=MainStates.shownMovieDetails)
# async def process_write_movie_review(message: Message, state: FSMContext):
#     logger.info(f'User @{message.chat.username} wants to add a review for a movie')
#     # TODO: add later
...
async def process_actor_details(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        msg = f'{r["name"]} ({r["enName"]})\nПол: {r["sex"]}\n' \
              ...
              f'Место рождения: {r["birthPlace"] if r["birthPlace"] else "нет данных"} ' \
            # f'Фильмы: {", ".join([x["name"]])}'
        # msg = f'{actors[num]["description"]}\n\n' \
        #       f'Рейтинг Кинопоиска: {actors[num]["rating"]["kp"] if actors[num]["rating"]["kp"] > 0 else "нет информации"}\n' \
        #       f'Рейтинг IMDB: {actors[num]["rating"]["imdb"] if actors[num]["rating"]["imdb"] else "нет информации"}'
        # await MainStates.shownMovieDetails.set()
        await MainStates.initial.set()
        if r["photo"]:
