...
async def start(message: Message):
    ...
    msg = """
    👋 Привет!
    Я могу помочь тебе найти какой-нибудь фильм, актёра или узнать прогноз погоды.
    """
    await MainStates.initial.set()
    await message.bot.send_message(message.chat.id, msg)


greetings_phrases = ["привет", "здравствуйте", "добрый день", "доброе утро", "добрый вечер"]
movie_phrases = ["фильм", "кино", "фильмец"]
actor_phrases = ["актер", "актёр", "актриса", "актера", "актёра", "актрисой", "актером", "актёром", "актрисой"]
